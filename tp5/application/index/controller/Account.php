...
class Account extends Controller
{
    ...
    public function addgoods()
    {
        ...
        if($gid && $name && $price && $amount && $category && $image)
		{

			$regid = Db::table('goods')->where('gid', $gid)->find();
			
			if($regid)
			{    
				$this->error( '该编号已存在，请重新添加！',url('/index/common/addgoods'));
			}
			else
			{
				Db::table('goods')
