...
class EasyBlogViewLatest extends EasyBlogView
{
	...
	public function display($tmpl = null)
	{
		...
		$customCano = $themes->getParam('seo_custom_canonical', '');
		if ($customCano) {
			$canoRoute = false;
			$canoLink = $customCano;

			if (strpos('?', $canoLink) === false) {
				$canoConcate = '?';
			}
		}

		$canoLink .= ($limitstart) ? $canoConcate . 'limitstart=' . $limitstart : '';
		$this->canonical($canoLink, $canoRoute);
